...
def copy_set(vis_ids, dirpath, target_dir, to_set='train'):
    ...
    for vis_id in vis_ids:
        vis_defects = np.unique(glob(osp.join(dirpath, 'images', f'*vialID{int(vis_id)}*')))
...
def split_before_move(datapath, target_path, cam_dict):
    for dirpath, dirnames, fname in os.walk(datapath):
        ...
        # if 'CnC' in dirpath:
        #     print(f'{dirpath}')
        if 'images' in dirnames and f'{os.sep}train{os.sep}' in dirpath and not osp.exists(target_dir := osp.join(target_path, dirpath[len(datapath)+1:])):
            train_split = 0.2
            val_split = 0.4
...
if __name__ == '__main__':
    ...
    target_path = '/nn-seidenader-gentofte/tjsd/Data/Real'
