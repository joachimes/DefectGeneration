...
def fill_dict(len_vis_txt, folder_class, git_hash, version, master_dict, sepsplit, origin, def_dict):
    split = 'train' if 'train' in sepsplit.lower() else 'val' if 'val' in sepsplit.lower() else 'test'
    ...
    if origin not in master_dict[split][folder_class]:
        ...
        master_dict[split][folder_class][origin]['split'] = sepsplit
...
def add_real_good(path:str, master_dict:dict, cam:int, def_dict:dict, splits:list=['train']):
    target_origin = 'Real'
    for split in splits:
...
def read_files(path:str, def_dict:dict, cam:int, origins:list):
    ...
    master_dict = generate_sets(path, master_dict, ['Real'], cam, def_dict, gen_sets=[ 'test'])
    master_dict['train'] = {}
    master_dict = generate_sets(path, master_dict, origins, cam, def_dict, gen_sets=['synthTrain', 'synthVal'])
    # import copy

    # master_dict['test'] = copy.deepcopy(master_dict['val'])
    # ensure that all classes are in all splits else pop
    if 'Real' not in origins:
        master_dict = add_real_good(path, master_dict, cam, def_dict, splits=['train', 'val'])
    ...
    dest_path = osp.join('config','data_config',f'CAM{cam}',f'{"_".join(origins)}_generative.yaml')
...
if __name__ == '__main__':
    ...
    for origins in origins_combinations: 
        for cam in [2]:
            ...
            path = f'/nn-seidenader-gentofte{os.sep}TJSD{os.sep}VisData'
