...
def sample(cfg, model, defect_info, model_version, num_images=100, batch_size=1, steps=50, eta=0., scale=7.0, strength=0.8):
    device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
    model = model.to(device)
    # indir = f'/nn-seidenader-gentofte/TJSD/VisData/Real/CAM{cfg.dataset.camera}/Good/'
    outdir = f'/nn-seidenader-gentofte/TJSD/VisData//CAM{cfg.dataset.camera}'
