...
def sample(cfg, model, defect_info, model_version, num_images=1000, batch_size=4, steps=49, n_row=4):
        ...
        with model.ema_scope():
            for origin in [ 'Synthetic']:
                ...
                for defect_name in sorted(os.listdir(indir)):
                    ...
                    outpath = osp.join(outdir, defect_name, model_version+f'_img2img_steps={steps}', 'train', 'images')
                    ...
                    for i in tqdm(range(0, num_images, batch_size)):
                        ...
                        samples, _ = sampler.sample(S=steps, batch_size=batch_size, shape=latent_shape
                                                    , conditioning=batch_input, verbose=False, x_T=x_T)
                        ...
                        for j in range(i, i+batch_size):
                            ...
                            Image.fromarray(samples[j%batch_size].astype(np.uint8)).save(osp.join(outpath, f'{j:04d}.jpg'))
                            Image.fromarray(img_batch[j%batch_size].astype(np.uint8)).save(osp.join(cond_img_path, f'{j:04d}.jpg'))
